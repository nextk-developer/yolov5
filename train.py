...
def train(hyp, opt, device, callbacks, run):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if RANK in {-1, 0}:
            ...
            # mlflow : log metric
            mlflow.log_metric('train/box_loss', np.nan_to_num(log_vals[0]).item(), step=epoch)
            mlflow.log_metric('train/obj_loss', np.nan_to_num(log_vals[1]).item(), step=epoch)
            mlflow.log_metric('train/cls_loss', np.nan_to_num(log_vals[2]).item(), step=epoch)
